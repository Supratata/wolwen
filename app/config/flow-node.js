...
module.exports = flow = function(temporaryFolder) {
    ...
    $.post = function(req, callback) {
        ...
        if (validation == 'valid') {
            var chunkFilename = getChunkFilename(chunkNumber, identifier);
            var subFolder = path.normalize(req.route+relativePath);
            var fold = subFolder.split('\\');
            fold.splice(fold.length-1,1);
            console.log('fold',fold);
            if(!fs.existsSync(subFolder)){
                mkdirp.sync(fold.join('\\'));
            }
                    ...
                    fs.exists(getChunkFilename(currentTestChunk, identifier), function(exists){
                        if(exists){
                            ...
                            if(currentTestChunk>numberOfChunks) {
                                ...
                                callback('done', filename, original_filename, identifier, currentTestChunk, numberOfChunks, subFolder);
                            } else {
                                // Recursion
                                testChunkExists();
                            }
                            ...
                            //Add currentTestChunk and numberOfChunks to the callback
                            callback('partly_done', filename, original_filename, identifier, currentTestChunk, numberOfChunks, subFolder);
                        }
                    });
                };
                testChunkExists();
